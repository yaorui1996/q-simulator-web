...
import { Display, Gate, getGateID, isGateInPaletteDropzone, isGateInCircuitDropzone, isGateValid, initGateBlank, setGateValue, isGateEqual } from './Gate'
...
import Wires from './icons/Wires.vue'
...
<template>
    <div class="circuit-dropzone">
        <Wires class="circuit-wires" v-if="isGateInCircuitDropzone(gate)" :wire-input="gate.wireInput"
            :wire-output="gate.wireOutput" :connect-top="gate.connectTop" :connect-bottom="gate.connectBottom" />
        <div class="gate-container" :class="gate.name == 'Null' ? 'z-0' : 'z-10'" @mouseover="mouseover"
            @mouseleave="mouseleave" @mousedown="mousedown" @mouseup="mouseup">
            <component :is="gateElements[gate.name]" :value="gate.value" :display="gate.display" :in-circuit-dropzone="isGateInCircuitDropzone(gate)" />
        </div>
    </div>
</template>
