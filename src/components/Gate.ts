...
export function setGateName(gate1: Gate, gate2: Gate): void {
  gate1.name = gate2.name
}

export function setGateDisplay(gate: Gate, display: Display): void {
  gate.display = display
}

export function setGateValue(gate1: Gate, gate2: Gate): void {
...
export function arrangeRegister(circuit: Gate[][], registerMin: number): void {
  while (getRegisterNum(circuit) > registerMin) {
    const lastRegisterGateNames: string[] = circuit.map((step) => step[step.length - 1].name)
...
export function arrangeWires(circuit: Gate[][]): void {
  if (getRegisterNum(circuit) > 0) {
    const wires: boolean[] = Array(getRegisterNum(circuit)).fill(false)
    ...
    circuit.forEach((step) => {
      ...
      const names: string[] = step.map((gate) =>
        uncontrollableGate.concat(['Null', 'Control']).includes(gate.name) ? gate.name : 'Controllable'
      )
...
export function appendRegister(circuit: Gate[][]): void {
  const i: number = getRegisterNum(circuit)
